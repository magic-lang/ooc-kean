...
ThreadPoolTest: class extends Fixture {
	init: func {
		...
		this add("wait with timeout", func {
			...
			expect(promise wait(TimeSpan milliseconds(10)) == false)
			expect(future wait(TimeSpan milliseconds(10)) == false)
			expect(promise wait() == true)
			expect(future wait() == true)
			expect(promise2 wait(TimeSpan seconds(10)) == true)
			expect(future2 wait(TimeSpan seconds(10)) == true)
			promise free()
			promise2 free()
			future free()
			future2 free()
