...
//			expect(FloatSize2D parse(value), is equal to(this vector3))
		...
		this add("minimum maximum", func {
			...
			expect(_max width, is equal to(22.0f) within(this precision))
			expect(_max height, is equal to(13.0f) within(this precision))
			expect(_max depth, is equal to(20.0f) within(this precision))
			expect(_min width, is equal to(12.0f) within(this precision))
			expect(_min height, is equal to(-3.0f) within(this precision))
			expect(_min depth, is equal to(10.0f) within(this precision))
		})
		this add("rounding", func {
			_round := this vector4 round()
			_ceiling := this vector4 ceiling()
			_floor := this vector4 floor()
