...
FloatComplexVectorListTest: class extends Fixture {
	...
	init: func {
		...
		this add("discrete fourier transform", func {
			...
			result2 := FloatComplexVectorList inverseDiscreteFourierTransform(result)
			for (i in 0 .. (this complexNumberArray count))
				expect((result2[i] - this complexNumberArray[i]) absoluteValue, is less than(tolerance))
			(result, result2) free()
		})
		this add("fast fourier transform", func {
			...
			expect((result[0] - FloatComplex new(8, 5)) absoluteValue, is less than(tolerance))
			expect((result[1] - FloatComplex new(0, -7)) absoluteValue, is less than(tolerance))
			expect((result[2] - FloatComplex new(6, 3)) absoluteValue, is less than(tolerance))
			expect((result[3] - FloatComplex new(-6, 3)) absoluteValue, is less than(tolerance))
			for (i in 0 .. result count)
				expect((result[i] - resultInPlace[i]) absoluteValue, is less than(tolerance))
			result2 := FloatComplexVectorList inverseFastFourierTransform(result)
			for (i in 0 .. (this complexNumberArray count))
				expect((result2[i] - this complexNumberArray[i]) absoluteValue, is less than(tolerance))
			...
			for (i in 0 .. (this complexNumberArray count))
				expect((result[i] - resultInPlace[i]) absoluteValue, is less than(tolerance))
			(fftBuffer, result, result2, resultInPlace) free()
		})
		...
		this add("toString", func {
			...
			text free()
			text = list toString(", ")
			expect(text, is equal to("-1.00 +2.00i, 3.00 -4.00i, -5.00 +6.00i"))
			(text, list) free()
		})
