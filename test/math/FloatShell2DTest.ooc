...
FloatShell2DTest: class extends Fixture {
	init: func {
		...
		this add("increase, decrease (box)", func {
			...
			expect(inc left, is equal to(0.0f) within(tolerance))
			expect(inc top, is equal to(1.0f) within(tolerance))
			expect(inc right, is equal to(5.0f) within(tolerance))
			expect(inc bottom, is equal to(7.0f) within(tolerance))
			expect(dec left, is equal to(2.0f) within(tolerance))
			expect(dec top, is equal to(3.0f) within(tolerance))
			expect(dec right, is equal to(3.0f) within(tolerance))
			expect(dec bottom, is equal to(5.0f) within(tolerance))
		})
		this add("increase, decrease (size)", func {
			...
			expect(inc left, is equal to(-1.0f) within(tolerance))
			expect(inc top, is equal to(-1.0f) within(tolerance))
			expect(inc right, is equal to(2.0f) within(tolerance))
			expect(inc bottom, is equal to(3.0f) within(tolerance))
			expect(dec left, is equal to(1.0f) within(tolerance))
			expect(dec top, is equal to(1.0f) within(tolerance))
			expect(dec right, is equal to(0.0f) within(tolerance))
			expect(dec bottom, is equal to(1.0f) within(tolerance))
		})
	}
}
