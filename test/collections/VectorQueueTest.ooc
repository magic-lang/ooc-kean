...
VectorQueueTest: class extends Fixture {
	init: func {
		...
		this add("Dequeue using default value", func {
			...
			expect(queue peek~default(MyCover new(0)) content, is equal to(1))
			expect(queue dequeue~default(MyCover new(7)) content, is equal to(1))
			expect(queue dequeue~default(MyCover new(3)) content, is equal to(2))
			expect(queue empty, is true)
			expect(queue dequeue~default(MyCover new(4)) content, is equal to(4))
			queue free()
		})
		this add("Dequeue using default null", func {
			...
			expect(queue peek~default(MyClass new(5)) content, is equal to(1))
			expect(queue dequeue~default(MyClass new(7)) content, is equal to(1))
			expect(queue dequeue~default(MyClass new(3)) content, is equal to(2))
			expect(queue empty, is true)
			expect(queue dequeue~default(null), is equal to(null))
			queue free()
		})
	}
