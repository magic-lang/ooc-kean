...
FloatRotation3DTest: class extends Fixture {
	...
	init: func {
		...
		this add("sphericalLinearInterpolation", func {
			...
			expect(interpolated _quaternion w, is equal to(0.207024594642948f) within(tolerance))
			expect(interpolated _quaternion x, is equal to(0.f) within(tolerance))
			expect(interpolated _quaternion y, is equal to(0.686707316993361f) within(tolerance))
			expect(interpolated _quaternion z, is equal to(0.696831312442762f) within(tolerance))
		})
		this add("angle", func {
