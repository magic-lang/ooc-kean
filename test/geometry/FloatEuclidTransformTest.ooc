...
FloatEuclidTransformTest: class extends Fixture {
	init: func {
		...
		this add("convolveCenter translations 1", func {
			...
			for (i in 0 .. 5) {
				euclidTransforms add(FloatEuclidTransform new(FloatVector3D new(i, i, i), Quaternion identity))
				kernel add(0.2f)
			}
		...
		this add("convolveCenter rotations", func {
			...
			kernel := FloatVectorList gaussianKernel(5)
			for (i in 0 .. 5) {
				rotation := Quaternion createRotationZ(i)
			...
			expect(result rotation distance(expectedRotation), is equal to (0.0f) within(tolerance))

			(kernel, quaternions, euclidTransforms) free()
		})
		this add("inverse", func {
			transform := FloatEuclidTransform new(FloatVector3D new(), Quaternion createRotationZ(3))
			...
			(translation, quaternion) := (first translation as FloatVector3D, first rotation as Quaternion)
			expect(translation norm, is equal to(0.f))
			expect(quaternion isIdentity, is true)
		})
		this add("mix", func {
			transform1 := FloatEuclidTransform new()
			transform2 := FloatEuclidTransform new(FloatVector3D new(1.f, 2.f, 3.f), Quaternion createFromEulerAngles(0.1f, 0.2f, 0.3f), 0.95f)
			...
			expect(mixedTransform rotation, is equal to(Quaternion new(0.9958282709f, 0.0172071829f, 0.0532323383f, 0.0720868334f)))
