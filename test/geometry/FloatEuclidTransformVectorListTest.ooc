...
FloatEuclidTransformVectorListTest: class extends Fixture {
	init: func {
		...
		this add("get and set", func {
			...
			euclidTransform1 := FloatEuclidTransform new(FloatVector3D new(1.0f, 2.0f, 3.0f), Quaternion createRotationX(1.0f))
			euclidTransform2 := FloatEuclidTransform new(FloatVector3D new(5.0f, 6.0f, 7.0f), Quaternion createRotationX(1.0f))
		...
		this add("get lists", func {
			...
			rotation1 := Quaternion new(1.21f, 2.31f, 3.14f, 4.23f)
			rotation2 := Quaternion new(2.51f, 3.12f, 4.41f, 5.42f)
			scaling1 := 5.72f
			scaling2 := 6.79f
			...
			expect(rotationX[0], is equal to(rotation1 eulerX) within(tolerance))
			expect(rotationX[1], is equal to(rotation2 eulerX) within(tolerance))
			expect(rotationY[0], is equal to(rotation1 eulerY) within(tolerance))
			expect(rotationY[1], is equal to(rotation2 eulerY) within(tolerance))
			expect(rotationZ[0], is equal to(rotation1 eulerZ) within(tolerance))
			expect(rotationZ[1], is equal to(rotation2 eulerZ) within(tolerance))
		...
		this add("convolve", func {
			...
			rotation1 := Quaternion createRotationZ(0.12f)
			rotation2 := Quaternion createRotationX(0.31f)
			rotation3 := Quaternion createRotationX(-0.63f)
			quaternionList := VectorList<Quaternion> new()
			quaternionList add(rotation1)
			quaternionList add(rotation1)
			quaternionList add(rotation2)
			filteredRotation0 := Quaternion weightedMean(quaternionList, kernel)
			quaternionList[0] = rotation1
			quaternionList[1] = rotation2
			quaternionList[2] = rotation3
			filteredRotation1 := Quaternion weightedMean(quaternionList, kernel)
			quaternionList[0] = rotation2
			quaternionList[1] = rotation3
			quaternionList[2] = rotation3
			filteredRotation2 := Quaternion weightedMean(quaternionList, kernel)
			scaling1 := 0.81f
			scaling2 := 0.43f
			scaling3 := 1.7f
