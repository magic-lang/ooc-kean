		...
		action := func (key: String, value: Object) {
			...
			if (value instanceOf?(Cell)) {
				...
				match (cell T) {
					...
					case IntSize2D => size := cell as Cell<IntSize2D> get(); program setUniform(key, size x, size y)
					case IntSize3D => size := cell as Cell<IntSize3D> get(); program setUniform(key, size x, size y, size z)
					...
					case FloatSize2D => size := cell as Cell<FloatSize2D> get(); program setUniform(key, size x, size y)
					case FloatSize3D => size := cell as Cell<FloatSize3D> get(); program setUniform(key, size x, size y, size z)
