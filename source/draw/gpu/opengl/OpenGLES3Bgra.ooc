...
OpenGLES3Bgra: class extends GpuBgra {
	...
	init: func ~fromPixels (size: IntSize2D, stride: UInt, data: Pointer) {
		super(size)
		this _backend = Texture create(TextureType bgra, size width, size height, stride, data)
	}
	create: func ~fromRaster (rasterImage: RasterBgra) -> This {
	...
	replace: func (image: RasterBgra) {
		this _backend uploadPixels(image pointer)
	}
	bind: func (unit: UInt) {
