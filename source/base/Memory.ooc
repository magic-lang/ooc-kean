...
Memory: cover {
	...
	init: func@ ~fromSize (size: Int) {
		this init(gc_malloc(size), size)
	}
	init: func@ ~fromData (=_pointer, =_size)
	free: func@ -> Bool {
		result: Bool
		if (result = this _pointer != null)
	...
	slice: func (start, distance: Int) -> This { // call by value -> modifies copy of cover
		...
		if (distance < 0)
			start -= size
		start < this size ? This new(this _pointer + start, Int minimum~two(size, this _size - start)) : This empty
	}
